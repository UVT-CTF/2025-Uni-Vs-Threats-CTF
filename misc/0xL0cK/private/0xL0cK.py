...
def list_commands():
    ...
    if unlocked:
        ...
        print("DECRYPT <file>  - Decrypt protected files")
...
def decrypt_file(file):
    if file == ".emergency_override":
        type_out(Fore.GREEN + "\n> Decryption requires a key:" + Style.RESET_ALL)
        addr = input("KEY> ")
        if addr == "ThisI5Th3K3yH4ppy?":
            type_out(Fore.GREEN + "> ADDRESS VERIFIED" + Style.RESET_ALL)
            type_out(Fore.GREEN + "> DECRYPTING .emergency_override..." + Style.RESET_ALL )
            time.sleep(2)
            type_out("")
            type_out(Fore.GREEN + "> Opening .emergency_override" + Style.RESET_ALL)
            time.sleep(1)
            type_out("")
            type_out(Fore.GREEN + """
